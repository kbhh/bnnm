...
class PlantViewModel(application: Application): ViewModel() {
    ...
    internal fun scheduleReminder(
        ...
        val  data  = Data.Builder().putString("WaterReminder.Worker.nameKey", plantName).build()
        // TODO: Generate a OneTimeWorkRequest with the passed in duration, time unit, and data
        //  instance
        val water = OneTimeWorkRequestBuilder<WaterReminderWorker>().setInitialDelay(duration, unit).setInputData(data).build()
        // TODO: Enqueue the request as a unique work request
        workManager.enqueueUniqueWork(plantName, ExistingWorkPolicy.REPLACE,water)
    }
}
